...
### Fixed
- **Compose SQLite volume was not writable by `appuser`** — the runtime image never created `/app/data`, so a named volume mounted at that path (`sqlite-data:/app/data`) was created `root:root` and uid 1001 could not open `memory.db`. The Dockerfile now `mkdir -p /app/data` before `chown`. Helm was unaffected (`fsGroup: 1001`). **Existing local volumes are not re-seeded:** `docker compose down -v` (destroys the SQLite file) or a manual `chown -R 1001:1001` on the volume. Smoke test now probes a named volume as uid 1001.
- **Cloud Terraform path could not reach a Running pod** (#127) — three independent configuration defects blocked the documented EKS/GKE/AKS apply path. (1) Entry-point headers referenced `infra/terraform/environments/*.tfvars` that did not exist; six non-secret tfvars are now committed (GKE/AKS account-specific fields called out at file heads). (2) Helm releases loaded `values.${environment}.yaml`, so `environment=dev` applied local-only `values.dev.yaml` (`image.pullPolicy: Never`) plus a bare `langgraph-agent-stack` image name — `ErrImageNeverPull` / `ImagePullBackOff` on a remote cluster. Modules now layer `values.cloud.yaml` (GHCR registry, tag omitted → Chart `AppVersion`) and optionally `values.prod.yaml` via `helm_values_files`; the chart also supports `image.pullSecrets` for private GHCR. (3) EKS had no EBS CSI driver while chart persistence defaults to enabled, so PVCs stayed Pending — `modules/eks` now installs the `aws-ebs-csi-driver` addon with dedicated IRSA. Also removes invalid GKE `lifecycle.prevent_destroy = var.environment == "prod"` (Terraform forbids variables there; GCP `deletion_protection` remains). Runbook: `docs/deploy.md`. Chart `version`/`appVersion` realigned from the stale `0.5.0` to `0.8.0` (the project had been on `0.7.0` since 2026-07-16 while the chart still declared `0.5.0`). The cloud overlay therefore resolves to `:0.8.0`, which exists on GHCR only once `v0.8.0` is released; before then, pin `image_tag` to a long git SHA (see `docs/deploy.md`).
- **FastAPI requests were never instrumented, so pack node spans had no parent** (#119) — `init_tracing()` set up the OTel SDK and `trace_span()` correctly opened per-node spans (`research_node`, `analysis_node`, ...), but nothing ever created a request-level span to nest them under. Every node span therefore arrived at the collector as a disconnected root, with no way to correlate them by request, see per-request status/duration, or tell which HTTP call a node belonged to. `api/lifespan.py` now calls the new `instrument_fastapi_app()` (`core/observability.py`) right after `init_tracing()`, wrapping `FastAPIInstrumentor().instrument_app()`. It is a no-op when tracing was not actually enabled (missing OTel SDK, `OTEL_ENABLED` unset) or when `opentelemetry-instrumentation-fastapi` is not installed, so the `observability` extra stays fully optional and no new dependency was needed (already declared in `pyproject.toml`). `/health`, `/ready`, and `/metrics` are excluded from instrumentation so probe traffic doesn't flood the collector. A new `set_span_attributes()` helper attaches `pack_id`/`run_id` to the active (request) span from `api/pack_execution.py::execute_typed_pack_run` and the typed/legacy stream and `/run` handlers, so a trace immediately shows which pack and run produced it. New tests in `tests/test_observability.py` cover the gating logic and, end-to-end, that a `trace_span()` call made during request handling nests under the request span created by instrumentation (same trace ID, correct parent span ID) and that excluded probe paths get no span at all.
...
### CI
- **Infra DevSecOps covers cloud overlays and Terraform entry points** (#127) — `scripts/infra-devsecops.sh` now renders and lint/validates default, `values.dev.yaml`, `values.cloud.yaml`, `values.prod.yaml`, and the cloud+prod stack (previously default + prod only). The Infra job validates `infra/terraform/{eks,gke,aks}` (not only modules), caches Terraform providers via `TF_PLUGIN_CACHE_DIR`, and the publish job also runs on `v*` tags with `type=semver` (`latest` remains main-only).
...
- **Budget exceeded on the synchronous path still returned HTTP 500** (#86 code review follow-up) — the node-level re-raise above escapes `astream_events()`/`stream_events()` fine, but `run()` in `research_analysis`/`research_only`/`analysis_only` wrapped *any* non-auth exception from `graph.invoke()` into a generic `AgentExecutionError`, silently re-boxing `AgentBudgetExceededError` before it could reach the `except AgentBudgetExceededError` → 402 handlers in `api/pack_execution.py` / `api/endpoints/pipeline.py`. `run()` now re-raises it unwrapped, same as `AgentAuthenticationError`. New `tests/test_budget_exceeded_e2e.py` drives the real compiled graph (mock provider + `LLM_COST_TABLE_PATH` override) end-to-end instead of mocking `astream_events`/`stream_events`, so a regression here fails a test again.
- **SSE `token` events could carry a list instead of text** — `chunk.content` from `on_chat_model_stream` may be a list of content blocks (Anthropic tool-use/thinking blocks) rather than a plain string; the `token` event now normalises it through the existing `extract_text_content()` helper so the SSE payload always carries text.
